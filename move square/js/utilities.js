...
function circlesIntersect(c1,c2) {
	var dx = (c2.x - (c1.x - (c1.width/2)));
	var dy = (c2.y - (c1.y - (c1.height/2)));

	if (dx > ((c1.width/2) + c2.radius)) { return false; }
    if (dy > ((c1.height/2) + c2.radius)) { return false; }

    if (dx <= (c2.width/2)) { return true; } 
    if (dy <= (c2.height/2)) { return true; }
	
	var dx=dx-c1.width/2;
    var dy=dy-c1.height/2;
